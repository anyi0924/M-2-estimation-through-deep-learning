...
modelPath = @(ep) fullfile(opts.saveDir, sprintf('FT-net-%d.mat', ep));
...
if exist(fullfile(opts.saveDir, sprintf('FT-net-%d.mat', opts.id)), 'file')
   ...
   if exist(fullfile(opts.dataDir, 'start_pattern_vgg.mat'), 'file')
        net = loadState(fullfile(opts.dataDir, 'start_pattern_vgg.mat')) ;
        fprintf('load start net model \n');
   else
       net = myDAGnet;
%        net.meta.imgRbgMean.r = 38.1396;
%        net.meta.imgRbgMean.g = 38.1396;
%        net.meta.imgRbgMean.b = 38.1396;
       net.meta.imgRbgMean.r = 0;
       net.meta.imgRbgMean.g = 0;
       net.meta.imgRbgMean.b = 0;
       saveState(fullfile(opts.dataDir, 'start_pattern_vgg.mat'), net, [], 0);
   end
end
...
for epoch=epoch_start+1:opts.numEpochs
    ...
    coslabel = zeros(1,1,2,10000,'single');
...
coslabel(1,1,:,ii)=single(train_label(:,ii))/3;
    end
  ...
  opts.train = opts.train(randperm(numel(opts.train))) ; % shuffle
  ...
  net.layers(36).block.setClip(opts.clip_d(epoch));
  ...
  fprintf(fid,'%s %.4f \n', '   train-L1_error = ', train_accuracy(1));
  fprintf(fid,'%s %.4f \n', '   train-L2_error = ', train_accuracy(2));
  val_accuracy = stats.val(epoch).accuracy;
  fprintf(fid,'%s %.4f \n', '   val-objective = ', stats.val(epoch).objective);
  fprintf(fid,'%s %.4f \n', '   val-L1_error = ', val_accuracy(1));
  fprintf(fid,'%s %.4f \n', '   val-L2_error = ', val_accuracy(2));
  fclose(fid);
...
net.layers(37).block.recalerror();
stats = opts.extractStatsFn(net) ;
